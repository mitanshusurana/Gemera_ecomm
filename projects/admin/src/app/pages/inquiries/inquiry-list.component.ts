  ...
  template: `
    <div class="p-6 max-w-7xl mx-auto">
      <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Custom Design Inquiries</h1>
      </div>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr *ngFor="let inq of inquiries()">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-medium text-gray-900">{{ inq.name }}</div>
                <div class="text-sm text-gray-500">{{ inq.email }}</div>
                <div class="text-sm text-gray-500">{{ inq.phone }}</div>
              </td>
              <td class="px-6 py-4">
                <div class="text-sm text-gray-900 max-w-xs truncate" [title]="inq.concept">{{ inq.concept }}</div>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <a *ngIf="inq.attachmentUrl" [href]="inq.attachmentUrl" target="_blank" class="text-indigo-600 hover:text-indigo-900">View File</a>
                <span *ngIf="!inq.attachmentUrl" class="text-gray-400">-</span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                  [ngClass]="{
                    ...
                    'bg-gray-100 text-gray-800': inq.status === 'CLOSED'
                  }">
                  {{ inq.status }}
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                <select [ngModel]="inq.status" (ngModelChange)="updateStatus(inq.id, $event)" class="text-sm border-gray-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <tr *ngIf="inquiries().length === 0">
              <td colspan="6" class="px-6 py-4 text-center text-sm text-gray-500">No inquiries found.</td>
            </tr>
          </tbody>
        </table>
      </div>
