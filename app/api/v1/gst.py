...
async def get_gstr1_data(
    ...
    hsn_summary = await db.execute(
        text("""
            SELECT
                '71131910' AS hsn_code, 'Gold Jewellery' AS description,
...
async def export_gstr1_json(period: str, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    ...
    b2b_res = await db.execute(text("""
        SELECT gst.invoice_no, gst.invoice_date, gst.party_gstin, gst.place_of_supply,
               gst.taxable_material_value + gst.taxable_making_value AS taxable_value,
               gst.cgst_amount, gst.sgst_amount, gst.igst_amount, gst.total_tax
        FROM caratloop.gst_output_tax_register gst
        WHERE gst.company_id = :cid AND gst.return_period = :period AND gst.supply_type = 'B2B'
        ...
        "b2b": [
            {
                ...
                "inv": [{
                    ...
                    "val": float(row["taxable_value"]) + float(row["total_tax"]),
                    "pos": row["place_of_supply"],
                    "rchrg": "N",
                    "itms": [{"num": 1, "itm_det": {"txval": float(row["taxable_value"]), "rt": 3, "camt": float(row["cgst_amount"]), "samt": float(row["sgst_amount"]), "iamt": float(row["igst_amount"])}}]
                }]
            } for row in b2b_rows
        ],
        "b2cl": [
            {
                ...
                "inv": [{
                    ...
                    "idt": str(row["invoice_date"]),
                    "val": float(row["taxable_value"]) + float(row["total_tax"]),
                    "itms": [{"num": 1, "itm_det": {"txval": float(row["taxable_value"]), "rt": 3, "iamt": float(row["igst_amount"])}}]
                }]
            } for row in b2c_rows
        ]
    }
