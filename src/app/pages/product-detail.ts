          ...
          <div class="lg:w-[42%] relative mt-8 lg:mt-0">
            <div class="sticky top-[110px] bg-white p-8 rounded-[18px] border border-[#e0e0e0] space-y-6">
              ...
              <div class="pb-6 border-b border-[#e0e0e0]">
                ...
                <!-- Share row (growth contract, section 2): quiet ghost pills under the price -->
                <div class="mt-4 flex flex-wrap items-center gap-2">
                  <span class="text-xs text-[#7a7a7a] mr-1">Share</span>
                  <a
                    [href]="whatsappShareUrl()"
                    target="_blank"
                    rel="noopener"
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-[#e0e0e0] text-xs font-medium text-[#1d1d1f] hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors active-press"
                  >
                  ...
                  <button
                    ...
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-[#e0e0e0] text-xs font-medium text-[#1d1d1f] hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors active-press"
                  >
                    <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M13.828 10.172a4 4 0 010 5.656l-3 3a4 4 0 01-5.656-5.656l1.5-1.5M10.172 13.828a4 4 0 010-5.656l3-3a4 4 0 015.656 5.656l-1.5 1.5"/></svg>
                    Copy link
                  </button>
                  <button
                    *ngIf="canNativeShare()"
                    ...
                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border border-[#e0e0e0] text-xs font-medium text-[#1d1d1f] hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors active-press"
                  >
                    <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" aria-hidden="true"><path stroke-linecap="round" stroke-linejoin="round" d="M4 12v7a1 1 0 001 1h14a1 1 0 001-1v-7M16 6l-4-4-4 4M12 2v13"/></svg>
                    Share
                  </button>
                </div>
              </div>
  ...
  implements OnInit, OnDestroy, AfterViewInit
{
  ...
  // Share row (growth contract, section 2)
  /** True only in a browser that implements the Web Share API; set in ngOnInit behind a platform guard. */
  canNativeShare = signal(false);

  /** Absolute product URL: the live origin in the browser, the canonical site origin during SSR. */
  shareUrl = computed(() => {
    const id = this.product()?.id;
    if (!id) return '';
    ...
    return this.seoService.absoluteUrl(path);
  });

  /** "<name> – <price> <url>" in the shopper's display currency. */
  private shareText = computed(() => {
    const p = this.product();
    if (!p) return '';
    const price = this.currencyService.format(this.currentPriceBreakup()?.total || this.currentPrice());
    return `${p.name} – ${price} ${this.shareUrl()}`.trim();
  });
  ...
  copyLink(): void {
    ...
    if (typeof navigator === 'undefined' || !navigator.clipboard?.writeText) {
      this.toastService.show('Copying is not available in this browser', 'error');
      return;
    }
    navigator.clipboard.writeText(url).then(
      () => this.toastService.show('Link copied', 'success'),
      () => this.toastService.show('Could not copy the link', 'error'),
    );
  }

  nativeShare(): void {
    const p = this.product();
    if (!isPlatformBrowser(this.platformId) || !p || typeof navigator === 'undefined' || !navigator.share) return;
    const price = this.currencyService.format(this.currentPriceBreakup()?.total || this.currentPrice());
    navigator
      .share({ title: p.name, text: `${p.name} – ${price}`, url: this.shareUrl() })
      .catch(() => {
        // The shopper closing the share sheet rejects with AbortError; nothing to report.
      });
  }
  ...
  ngOnInit(): void {
    ...
    // `navigator` does not exist during SSR; only probe for the Web Share API in a browser.
    if (isPlatformBrowser(this.platformId) && typeof navigator !== 'undefined' && typeof navigator.share === 'function') {
